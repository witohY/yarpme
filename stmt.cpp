...
StmtBlock::generateStructure(std::shared_ptr<GenCtx> ctx) {
    ...
    Statistics &stats = Statistics::getInstance(); //跟踪生成的语句数

    std::shared_ptr<Stmt> new_stmt;  //创建新语句，递归生成
    ...
    return std::make_shared<StmtBlock>(stmts); //指向语句向量的指针 是 StmtBlock
}
...
ScopeStmt::generateStructure(std::shared_ptr<GenCtx> ctx) {
    ...
    auto stmt_block = StmtBlock::generateStructure(std::move(ctx));//调用迭代生成
    new_scope->stmts = stmt_block->getStmts();  //std::shared_ptr<StmtBlock>
    return new_scope;
}
...
void LoopHead::createPragmas(std::shared_ptr<PopulateCtx> ctx) {
    Options &options = Options::getInstance();
    if (!options.isCXX() || EmitPragmas() == OptionLevel::NONE)
        return;
