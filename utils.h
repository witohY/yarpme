...
// According to the agreement（协议）, Random Value Generator is the only way to get any
...
class RandValGen {
  public:
    ...
    explicit RandValGen(uint64_t _seed);  //特定种子去复现
    ...
    template <typename T> T getRandValue(T from, T to) {
        ...
        //均匀分布的整数：std::uniform_int_distribution;
        //均匀分布的小数：std::uniform_real_distribution;
        //正态分布：std::normal_distribution;
        //对数分布：std::lognormal_distribution;
        std::uniform_int_distribution<long long> dis(from, to);  //取数器(范围)
    ...
    // Randomly chooses one of IDs, basing on std::vector<Probability<id>>. 得到类型ID
  ...
  private:
    ...
    // Auxiliary random generator, used for mutation  辅助 用于突变
...
class NameHandler {
  ...
  private:
    ...
    uint32_t var_idx;
    uint32_t arr_idx;
    uint32_t iter_idx; //表示循环变量的索引号，用于标识在某个循环中声明的循环变量。 ???迭代器
    uint32_t stub_stmt_idx;  //表示“存根”语句的索引号，用于标识在某个上下文中插入的占位语句，以便稍后在编译时填充代码。
};
} // namespace yarpgen
