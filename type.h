...
// Abstract class that serves as a common ancestor for all types. 所有类的父类
//转换为IR ???
// Here by "type" we mean a backend type, which has certain properties and may be instantiated as different language types
// (i.e. signed 32 bit integer, which may be int, or int32_t).
// For now we support only one language type per backend type, but that may change in future.
class Type {
  public:
    Type()
...
// Base class for floating-point and integral types  基础的浮点和整数类型
...
class IntegralType : public ArithmeticType {
  public:
    ...
    // represent all the values of type b  类型a是否可以表示类型b包含的所有值
    ...
    // unsigned type  查找无符号类型的算数转换的辅助函数
...
class ArrayType : public Type {
  ...
  private:
    ...
    // to each of them and then compare it.  为每个数组类型设定位移标识符
    static size_t uid_counter;  

    std::shared_ptr<Type> base_type;
    // Number of elements in each dimension  维度?????
