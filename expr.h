...
//表达式相关类的共同祖先
class Expr : public IRNode {  //结果  UB  类型转换
  public:
    ...
    //进行类型转换，在字节点和当前节点之间插入TypeCastExpr
    ...
    //根据子节点计算当前节点，检测UB，需要首先调用propagate_type()函数
  ...
  private:
    // TODO: add complexity tracker 复杂性跟踪器记录程序和函数中的表达式数量
    /*
    uint32_t complexity;
...
class ArrayStencilParams;  //数组模版的参数？？？
