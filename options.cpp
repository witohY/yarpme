...
std::vector<OptionDescr> yarpgen::OptionParser::options_set{
    ...
    {OptionKind::STD,
     ...
     "c++",
     {"c", "c++", "ispc", "sycl"}},   //ispc:加速图像处理,类C语法  sycl:SYCL是OpenCL的高级编程模型
    ...
    {OptionKind::INP_AS_ARGS,
     ...
     "some",
     {"none", "some", "all"}},   //将输入的数据作为参数传入
    {OptionKind::EMIT_ALIGN_ATTR,
     "",
     "--emit-align-attr",
     true,
     "Emit \"aligned\" attributes for arrays", //为数组发射“对齐”属性
    ...
    {OptionKind::UNIQUE_ALIGN_SIZE,
     ...
     "--unique-align-size",
     false,
     "Use unique align size for all attributes",   //对所有属性使用唯一的对齐大小
    ...
    {OptionKind::ALIGN_SIZE,
     ...
     "--align-size",
     true,
     "Size for \"aligned\" attributes for arrays",  //为数组大小设定基准
    ...
    {OptionKind::ALLOW_DEAD_DATA,
     ...
     "--allow-dead-data",
     true,
     "Allow to create data(vars and arrays) that will be never used",   //允许创建死数据
    ...
    {OptionKind::OUT_DIR,
     ...
     "--out-dir",
     true,
     "Folder for generated test files (it should exist)",  //指定输出的目标文件夹
     "Unreachable Error",
    ...
    {OptionKind::PARAM_SHUFFLE,
     ...
     "--param-shuffle",
     true,
     "Use parameters shuffling",                        //使用参数洗牌
    ...
    {OptionKind::EXPL_LOOP_PARAM,
     ...
     "--expl-loop-param",
     true,
     "Emit explicit loop's parameters",                  //发出显示的循环参数？？？？
    ...
    {OptionKind::MUTATE,
     ...
     "--mutate",
     true,
     "Test mutation type. Can be disabled, set to mutate expressions "         //测试突变类型。可以禁用，设置为变异表达式  ？？？？？？？？
    ...
    {OptionKind::UB_IN_DC,
     ...
     "--allow-ub-in-dc",
     true,
     "Whether we allow Undefined Behavior in Dead Code ",      //我们是否允许死代码中的未定义行为
